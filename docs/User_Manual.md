...
FACREDIT is a web-based Faculty Credit Management System designed for educational institutions. It enables teachers to submit leave requests, track their academic credit balance, and view their timetable — all in a single, easy-to-use interface.

This manual is intended for **teachers** and **secondary administrators** who use FACREDIT on a day-to-day basis.

### Who This Guide Is For

| Role | Description |
|------|-------------|
| Teacher | Submits leave requests, views timetable, tracks credit balance |
| Secondary Admin | All teacher functions plus leave approval, substitute assignment, and credit management |
| Super Admin | All features including system settings and factory reset |
...
Open your web browser and navigate to the FACREDIT URL provided by your institution (e.g., `http://localhost:5173` for local installations, or the production domain configured by your administrator).
...
FACREDIT sends in-app notifications for key events. A notification badge appears on the bell icon in the top navigation bar when you have unread notifications.
...
If your administrator has enabled VAPID push notifications, you may be prompted by your browser to allow notifications. Accepting this allows FACREDIT to send notifications even when the browser tab is in the background.
...
The Academic Calendar defines which dates are working days, holidays, exam days, or other special event types. Every FACREDIT operation — leave submission, timetable generation, substitute assignment — is governed by the Academic Calendar.
...
FACREDIT does not provide a self-service password reset email. If you have forgotten your password:
...
A: FACREDIT uses a rotating Day Order (DO1–DO6) system because the schedule does not follow a fixed Monday–Friday pattern. The system administrator assigns a Day Order to each working date in the Academic Calendar, and your timetable applies based on today's Day Order.
