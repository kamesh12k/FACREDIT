...
FACREDIT is a production-ready, role-based Faculty Credit Management System designed for educational institutions. It manages teacher leave requests, substitute assignments, and credit-based workload balancing through a rotating **Day Order (1–6)** schedule of five periods per day.
...
```powershell
git clone https://github.com/kamesh12k/FACREDIT.git
cd FACREDIT\credits-system
```
...
Python virtual environments isolate FACREDIT dependencies from the system Python. **Always use a virtual environment.**
...
### 7.4 Apply the Leave Status Enum Fix

```bash
psql -U postgres -d credits_db -c "ALTER TYPE leave_status ADD VALUE 'cancelled';"
```
...
Create `/etc/systemd/system/facredit.service`:

```ini
[Unit]
Description=FACREDIT Backend
After=network.target postgresql.service

[Service]
User=www-data
WorkingDirectory=/opt/facredit/credits-system/backend
EnvironmentFile=/opt/facredit/credits-system/backend/.env
ExecStart=/opt/facredit/credits-system/backend/venv/bin/gunicorn \
    app.main:app \
    --workers 4 \
    --worker-class uvicorn.workers.UvicornWorker \
    --bind 0.0.0.0:8000 \
...
sudo systemctl enable facredit
...
server {
    ...
    location / {
        root /opt/facredit/credits-system/frontend/dist;
...
FACREDIT automatically writes a timestamped JSON backup to `backend/backups/` whenever a **Factory Reset** is triggered. This backup captures all faculty data before deletion.
...
```bash
pg_dump -U postgres -F c -b -v -f "facredit_backup_$(date +%Y%m%d_%H%M%S).dump" credits_db
```
...
0 2 * * * pg_dump -U postgres -F c credits_db > /opt/facredit/backups/facredit_$(date +\%Y\%m\%d).dump
```
...
# Restore
pg_restore -U postgres -d credits_db -v "facredit_backup_YYYYMMDD_HHMMSS.dump"
...
```bash
psql -U postgres -d credits_db < "facredit_backup_YYYYMMDD_HHMMSS.sql"
```
...
```bash
sudo systemctl restart facredit
```
...
| GitHub Repository | https://github.com/kamesh12k/FACREDIT |

> **Note:** FACREDIT is proprietary software. Use, modification, or distribution without a commercial license agreement is prohibited. See `README.md` for full copyright terms.
