...
def is_teacher_self_management_allowed(db: Session) -> bool:
    campus_mode = get_campus_mode(db)
    if campus_mode == "autonomous":
        return False
    return get_setting(db, "teacher_self_management_enabled", "false") == "true"

def check_teacher_self_management_allowed(db: Session) -> None:
    if not is_teacher_self_management_allowed(db):
        raise HTTPException(
            status_code=400,
...
def teacher_get_leave_requests(db: Session, teacher_id: int) -> list[LeaveRequest]:
    check_teacher_self_management_allowed(db)
...
def teacher_get_candidates(db: Session, leave_id: int, teacher_id: int) -> list:
    ...
    return substitution_service.get_ranked_recommendations(db, leave_id)
