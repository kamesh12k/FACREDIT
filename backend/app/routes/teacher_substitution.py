...
@router.get("/enabled")
def check_enabled(db: Session = Depends(get_db)):
    enabled = service.is_teacher_self_management_allowed(db)
...
def get_config(
    ...
    enabled = get_setting(db, "teacher_self_management_enabled", "false") == "true"
...
def update_config(
    ...
    set_setting(db, "teacher_self_management_enabled", val)
    
    log_audit_event(
        db, super_admin.id, "teachers_mode.config_change", "system_setting", None,
        {
